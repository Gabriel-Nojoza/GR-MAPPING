...
import { BarChart3, Building2, CreditCard, FileSignature, LayoutDashboard, LogOut, Search, Settings, ShieldCheck, UsersRound } from "lucide-react";
...
export function AdminSidebar() {
  ...
  return <SidebarContainer>
    ...
    <SidebarNav>
      ...
      {ITENS_ADMIN.slice(0, 5).map((item) => <SidebarItem key={item.href} href={item.href} icon={<item.icon size={18} strokeWidth={2} />} label={item.label} active={pathname === item.href} />)}
      <p className="px-5 pb-2 pt-5 text-[10px] font-semibold uppercase tracking-[0.14em] text-white/35">Plataforma</p>
      {ITENS_ADMIN.slice(5).map((item) => <SidebarItem key={item.href} href={item.href} icon={<item.icon size={18} strokeWidth={2} />} label={item.label} active={pathname === item.href} />)}
    </SidebarNav>
