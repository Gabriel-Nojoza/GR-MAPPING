...
export async function atualizarRecursoEng(tipo: string, id: string, dados: { nome: string; dados: Record<string, string> }) { return financeiroResposta(await fetch(`${API_URL}/eng/recursos/${tipo}/${id}`, { method: "PATCH", headers: authHeaders(), body: JSON.stringify(dados) })) as Promise<RecursoEng>; }
...
export async function getPessoasEmObra() { return financeiroResposta(await fetch(`${API_URL}/eng/pessoas-em-obra`, { headers: authHeaders(), cache: "no-store" })) as Promise<PessoasEmObra[]>; }
